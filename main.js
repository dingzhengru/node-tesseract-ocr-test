...
//* 可以利用反轉圖片顏色，讓圖片辨識更準確 (invert 反轉圖片顏色之類的)
...
async function main() {
  ...
  const imgPath = 'image/18-plus-16.png';
  ...
  //* 灰階化 + 提高對比度 (提高準確度)，白底黑字辨識度較好
  const img = await jimpImage.invert().greyscale().contrast(1).write('image/img--jimp.png').getBase64Async(Jimp.AUTO);
